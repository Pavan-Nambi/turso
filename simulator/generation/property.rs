    ...
    pub(crate) fn interactions(
        ...
        let interactions: Vec<InteractionBuilder> = match self {
            ...
            Property::ReadYourUpdatesBack {
                ...
                let begin_tx =
                    InteractionType::Query(Query::Begin(sql_generation::model::query::transaction::Begin::Immediate));
                let update_interaction = InteractionType::Query(Query::Update(update.clone()));
                let commit_tx =
                    InteractionType::Query(Query::Commit(sql_generation::model::query::transaction::Commit));
                                            ...
                                            update.set_values.iter().enumerate()
                                        {
                                            match set_val {
                                                ...
                                                SetValue::CaseWhen { then_value, .. } => {
                                                    // For CaseWhen: value is either then_value or unchanged from before
                                                    let before_row =
                                                        before_rows.iter().find(|br| {
                                                            // Match rows by non-updated columns
                                                            row.iter()
                                                                .zip(br.iter())
                                                                .enumerate()
                                                                .filter(|(idx, _)| {
                                                                    !update.set_values.iter().any(
                                                                        |(c, _)| {
                                                                            // Check if this column index matches the set column
                                                                            update
                                                                                .set_values
                                                                                .iter()
                                                                                .position(
                                                    ...
                                                    let is_unchanged = before_row
                                                        .map(|br| row[i] == br[i])
                                                        .unwrap_or(false);
                                                    if !is_then && !is_unchanged {
                                                        return Ok(Err(format!(
                                                            "CaseWhen: row col {col} has unexpected value {} (expected {} or unchanged)",
                                                            row[i], then_value
                                                        )));
                                                    }
                                                }
