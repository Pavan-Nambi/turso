...
struct CommitInfo {
    ...
    published_batch_cursor: usize,
    published_frame_cursor: usize,
}
...
impl CommitInfo {
    fn reset(&mut self) {
        ...
        self.published_batch_cursor = 0;
        self.published_frame_cursor = 0;
    }
...
pub struct Pager {
    ...
    #[cfg(test)]
    commit_yield_after_wal_frame_cached: AtomicBool,
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            commit_info: RwLock::new(CommitInfo {
                ...
                published_batch_cursor: 0,
                published_frame_cursor: 0,
            }),
            #[cfg(test)]
            commit_yield_after_wal_frame_cached: AtomicBool::new(false),
        ...
        loop {
            ...
            match state {
                ...
                CommitState::WalCommitDone => {
                    ...
                    let mut commit_info = self.commit_info.write();
                    while commit_info.published_batch_cursor < commit_info.prepared_frames.len() {
                        let batch_idx = commit_info.published_batch_cursor;
                        let frame_idx = commit_info.published_frame_cursor;
                        if frame_idx >= commit_info.prepared_frames[batch_idx].metadata.len() {
                            commit_info.published_batch_cursor += 1;
                            commit_info.published_frame_cursor = 0;
                            continue;
                        }

                        let (page, frame_id, checksum) = {
                            let (page, frame_id, checksum) =
                                &commit_info.prepared_frames[batch_idx].metadata[frame_idx];
                            (page, *frame_id, *checksum)
                        };
                        wal.commit_prepared_frame(page, frame_id, checksum);
                        commit_info.published_frame_cursor += 1;
                        #[cfg(test)]
                        {
                            if self
                                .commit_yield_after_wal_frame_cached
                    ...
                    self.dirty_pages.write().clear();
                    commit_info.prepared_frames.clear();
                    commit_info.published_batch_cursor = 0;
                    commit_info.published_frame_cursor = 0;
...
mod tests {
    ...
    #[test]
    fn page_cache_does_not_duplicate_key_after_wal_frame_cache_yield() {
        let dir = tempfile::tempdir().unwrap();
        let db_path = dir.path().join("page-cache-duplicate-wal-frame-cache.db");
        ...
        exec_ok(
            &conn1,
            "DELETE FROM t WHERE k = x'77696c645f66727569745f363035'",
        );
        ...
        loop {
            match conn0_stmt.step().unwrap() {
                ...
                StepResult::Done => panic!("conn0 completed before WAL frame-cache yield fired"),
                StepResult::Row => {}
                StepResult::Interrupt | StepResult::Busy => {
                    panic!("conn0 stopped before WAL frame-cache yield fired")
                }
            }
        }
        ...
        exec_locked(
            &conn1,
            "DELETE FROM t WHERE k = x'6472795f626972645f363630'",
        );
        ...
        exec_locked(
            &conn1,
            "INSERT INTO t(k, v) VALUES (x'736f75725f626f6f6b5f393439', 139)",
        );
        exec_locked(
            &conn1,
            "DELETE FROM t WHERE k = x'636f6c645f706174685f373338'",
        );
        ...
        exec_ok(
            &conn1,
            "DELETE FROM t WHERE k = x'64697274795f666973685f353438'",
        );
        exec_ok(
            &conn1,
            "DELETE FROM t WHERE k = x'73776565745f77696e645f333836'",
        );
        exec_ok(&conn1, "PRAGMA data_sync_retry = 1");
        exec_ok(&conn1, "UPDATE t SET v = 895 WHERE k = zeroblob(1844)");
        exec_ok(&conn1, "BEGIN");
        exec_ok(&conn1, "INSERT INTO t(k, v) VALUES (zeroblob(7747), 422)");
